...
>   2. **Telegram 渠道**：将图片复制到 `~/.openclaw/workspace/xhs_qr.png`，然后用 `message` 工具的 `filePath` 参数直接把图片文件发送给用户（不依赖 Read tool 展示）
>   3. **其他渠道**：优先用 Read tool 读取图片文件，在当前会话中展示
>   4. 发送图片后，在同一回复中补充说明："请用小红书 APP 扫描二维码登录，登录后我会继续执行"
>   5. 如果图片读取失败，必须在文字回复中明确告知：二维码文件路径 + 下一步操作指引
> - **error_code=300012 IP风控识别与用户告知**：当脚本输出/页面 URL 包含 `error_code=300012` 或 `error_msg=IP存在风险` 时，必须立即停止登录重试流程：
>   1. **不得**继续发送二维码（否则循环失败）
>   2. 明确告知用户：`⚠️ 小红书拒绝了来自云服务器 IP 的请求（error_code=300012）——这是 IP 层面的风控，与账号无关，是云服务器出口 IP 被识别为机器人`
>   3. 提供解决方案选项：
>      - 住宅代理（推荐）：在 `chromium.launch()` 加 `proxy` 参数，Playwright 代码改动为零
>      - 换网络：用手机热点或其他家庭宽带环境
>   4. 询问用户选择哪种方案后再继续
