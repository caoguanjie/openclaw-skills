...
def cmd_check_cdp(args: argparse.Namespace) -> None:
    """检查 Chrome CDP 远程调试端口可用性。"""
    from xhs.cdp_check import check_cdp

    result = check_cdp(host=args.host, port=args.port)
    _output(result, exit_code=0 if result["cdp"] else 2)


def cmd_generate_excel(args: argparse.Namespace) -> None:
    """从 analysis.json 生成 Excel 潜客管理表。"""
    from xhs.excel_generator import generate_excel

    output_path, post_count, user_count, image_count = generate_excel(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # check-cdp
    sub = subparsers.add_parser("check-cdp", help="检查 Chrome CDP 远程调试可用性")
    sub.set_defaults(func=cmd_check_cdp)

    # generate-excel
    sub = subparsers.add_parser("generate-excel", help="从 analysis.json 生成 Excel 潜客管理表")
    sub.add_argument("--input", required=True, help="analysis.json 文件路径")
    sub.add_argument("--output", default=None, help="输出 Excel 路径（可选，默认自动生成）")
    sub.set_defaults(func=cmd_generate_excel)
