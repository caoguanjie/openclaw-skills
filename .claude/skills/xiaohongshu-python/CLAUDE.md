...
- `scripts/cli.py` — 统一 CLI 入口，25 个子命令，JSON 结构化输出
- `scripts/publish_pipeline.py` — 发布编排器（含图片下载和登录检查）
- `scripts/xhs/cdp_check.py` — CDP 连接检查
- `scripts/xhs/excel_generator.py` — Excel 潜客管理表生成（openpyxl）
- `references/site-patterns/` — 站点经验（小红书平台特征、有效模式、已知陷阱）
...
uv run python scripts/cli.py publish --title-file t.txt --content-file c.txt --images pic.jpg
uv run python scripts/publish_pipeline.py --title-file t.txt --content-file c.txt --images URL1
```
...
| `generate-excel` | — | 分析（analysis.json → Excel 潜客表） |
