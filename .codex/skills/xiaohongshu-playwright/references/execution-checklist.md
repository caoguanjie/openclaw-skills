...
- [ ] 已生成 `data/task-specs/<timestamp>_<keyword>.json`
- [ ] task spec 包含 `post_relevance / comment_filter / semantic_focus`
...
- [ ] `--task-spec` 参数已传入
- [ ] `data/candidates_<关键词>.json` 已生成
- [ ] 候选文件中包含 `taskSpecPath / taskSpec / posts / skippedPosts / stats`
- [ ] 终端输出的过滤统计合理

### 阶段 B: Claude 语义精筛
- [ ] 已读取 `candidates_<关键词>.json`
- [ ] 只对候选评论做语义精筛，不直接扫全量评论
- [ ] 每条评论有 score (1-10)、tags、reason
- [ ] `data/analysis.json` 已生成且格式正确
- [ ] 验证：抽查 3-5 条高分评论，确认判断合理

❌ **常见错误**:
- ⚠️ **禁止用关键词匹配代替语义分析** — task spec 只用于粗筛
- 遗漏子评论中的高价值用户
...
- [ ] 附带日期标记
- [ ] 成功完成后已清空 `data/task-specs/`
- [ ] 如果任务失败，已保留 task spec 供复盘
