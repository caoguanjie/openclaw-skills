...
- **修复**: Sprint 3.1 已完成，更新到 Chrome 131/132、Safari 18.x、Edge 131
...
- **修复**: Sprint 3.1 已完成，实现 getFingerprint() 函数动态匹配 platform
...
- **修复**: Sprint 3.1 已完成，getFingerprint() 随机返回 4/8/12/16 核心数和 4/8/16 GB 内存
...
| 优先级 | 编号 | 问题 | 影响 | 状态 |
|--------|------|------|------|------|
| P0 | #1 | `__INITIAL_STATE__` 数据断裂 | 可能导致大量评论丢失 | 待修复 |
| P0 | #2 | 无验证码检测 | 遇到验证码后盲目运行 | 待修复 |
| P0 | #3 | 不可访问帖子检测缺失 | 白白浪费时间 | 待修复 |
| P1 | #4 | UA 版本过时 | 易被反爬识别 | ✅ Sprint 3.1 |
| P1 | #5 | platform 硬编码 | 指纹矛盾 | ✅ Sprint 3.1 |
| P1 | #6 | 硬件参数固定 | 指纹一致 | ✅ Sprint 3.1 |
| P1 | #7 | rebrowser-patches 静默降级 | 反检测失效 | 待修复 |
| P1 | #8 | 崩溃丢数据 | 长任务可靠性差 | 待修复 |
| P2 | #9-12 | 数据质量 | 功能受限、数据不完整 | 待修复 |
| P2 | #13-16 | 健壮性 | 长时间运行不稳定 | 待修复 |
| P3 | #17-20 | 缺失功能 | 进阶用户需求 | 待修复 |
| P3 | #21-24 | 工程质量 | 维护成本高 | 待修复 |
